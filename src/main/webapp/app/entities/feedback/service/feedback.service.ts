...
type RestOf<T extends IFeedback | NewFeedback> = Omit<T, 'dataFeedback'> & {
...
export class FeedbackService {
  ...
  protected convertDateFromClient<T extends IFeedback | NewFeedback | PartialUpdateFeedback>(feedback: T): RestOf<T> {
    return {
      ...feedback,
      dataFeedback: feedback.dataFeedback?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFeedback: RestFeedback): IFeedback {
    return {
      ...restFeedback,
      dataFeedback: restFeedback.dataFeedback ? dayjs(restFeedback.dataFeedback) : undefined,
    };
  }
